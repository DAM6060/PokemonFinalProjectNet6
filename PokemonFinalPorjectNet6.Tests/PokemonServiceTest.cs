...
namespace PokemonFinalPorjectNet6.Tests
{
	public class PokemonServiceTest
	{
		...
		{
			var pokemon = await pokemonService.GetPokemonByIdAsync(4);
		...
		public async Task PlayerHasPokemonWithIdWorksCorrectly()
		{
			...
			var falseHasPokemon = await pokemonService.PlayerHasPokemonWithId(4, 1);
		...
		public async Task ExistsAsyncWorksCorrectly()
		{
			...
			var falseExists = await pokemonService.ExistsAsync(4);
			Assert.That(exists, Is.True);
			Assert.That(falseExists, Is.False);
		}
		[Test]
